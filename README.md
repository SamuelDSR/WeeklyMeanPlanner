...
> Household · Maisonnée —— 一家人的日子：吃什么、花了多少、卡在哪。
...
- 按食材分类分组，勾选有离线队列（断网也能勾，联网自动补发）→ 第 35 节
...
**记账**
- **支出和收入**都记，给出结余 → 第 31 节
- 记一笔是全屏的图标网格 + 自带加减的小键盘（早餐 21 + 午餐 35 直接敲）→ 第 31 节
- **子账本**：度假、装修这类单独拢起来，既能单独看，也仍算进总账 → 第 31 节
- **按月 / 按年**翻看，分类排行带占比条；多货币各算各的，**从不换算**

**卡包**
- 会员卡存成条码 / 二维码，点一下全屏放大给扫码枪 → 第 32 节
- **点商家 → 扫一下 → 就好了**：内置 30 家法国常见商超餐饮，名字和码制都不用填 → 第 32 节
...
**离线**
- 菜单 / 菜谱 / 购物 / 记账 / 卡包 **读取全部可离线** → 第 35 节
- 离线记的账、勾的清单会攒着，联网自动补发；带幂等键，重发不会记两笔
- 卡包本地优先：打开即显示，不等网络
...
**记一笔**是全屏面板，顺序照着人怎么想来排：

```
支出 / 收入        先定方向
  ↓
图标网格点一下      18 个支出分类 / 8 个收入分类，一行四个
  ↓
小键盘敲金额        7 8 9 今天 / 4 5 6 + / 1 2 3 − / . 0 ⌫ 完成
  ↓
完成
```

**金额支持直接加减**：早餐 21、午餐 35，敲 `21+35` 就行，右边实时显示 `= €56.00`，
不用先开计算器。只支持 + 和 −（记账用不到乘除），而且**绝不用 `eval`** ——
自己扫一遍字符串（`frontend/src/lib/expenseCalc.js`），`1+1;alert(1)` 这种直接返回 null。

分类做成图标网格而不是下拉框：十几个分类用下拉要滚要找，图标一眼扫过去就点中了。
记账这事一慢就不会坚持记。

**收入和支出分开算，再给结余：**

```
支出 €575.92   收入 €2 800.00   结余 €2 224.08
```

金额一律存正数，方向由 `kind` 决定。用正负号表方向的话，一个 `SUM` 就把收支
混成一个净额了 —— 而「这个月花了多少」和「这个月挣了多少」是两个问题。

分类也分收支两套：`工资` 记成支出、`餐饮` 记成收入都会被拒（`「工资」不是支出的分类`）。

**按月 / 按年**都能看，切换在页面顶上。分类排行带占比条：
...
> 占比只在**单一货币**时才算。同时有 EUR 和 CHF 的话，「餐饮占 30%」需要先换算，
> 而我们不换算 —— 那种情况下只列金额，不画条。

**子账本**是这个功能的另一半。去意大利待两周，开一个子账本，那两周的花销都记进去：
...
支持 Code 128 / EAN-13 / EAN-8 / UPC-A / Code 39 / ITF / QR / **PDF417**。全在浏览器里画
（一维码用 `jsbarcode`，QR 用 `qrcode`，PDF417 用 `pdf417-generator`），
所以**没网也能调出来**——装成 App 之后照样能用。

PDF417 是堆叠式二维码，又宽又扁（法国这边一些会员卡、驾照、登机牌在用）。
它和一维码一样，横过来更长更好扫，所以全屏视图里同样给了旋转按钮 —— 只有 QR 是方的，转了没意义。

> 画 PDF417 的时候要**自己铺一层白底**。编码器只画黑块，底子是透明的：
> 屏幕上靠 CSS 背景看着没问题，可一旦被截图或复制出去合成，就成了黑底黑码。
> 本仓库踩过这个坑（实测画完黑色占比是 1.0，也就是全黑）。
...
加一张
  ↓
选商家   Carrefour / E.Leclerc / Auchan / Super U / Picard / McDonald's /
         Leroy Merlin / Decathlon …（内置 30 家，可搜索）
  ↓
相机自动打开，对着卡上的条码
  ↓
存好了 —— 名字、品牌色、码制都不用填
...
码制是**扫出来的**，不用去猜这张卡是 EAN-13 还是 Code 128。
列表里没有的商家走「手动添加」，和以前一样。
扫不出来或者没扫成，会退回预填好商家的表单，不让你白点一趟。

商家表在 `server/src/cardBrands.js`，数据库里只存一个 slug —— 
名字、品牌色、首字母都在代码里，以后换配色或者补商家不用动数据库。
不放真 logo：版权是一回事，几十张图会把安装包撑大、离线还得缓存，
而且在货架前找卡，先认到的本来就是颜色。

**手动录卡时也能扫。** 卡号旁边有「扫一扫」—— 13 位数字手输很容易错一位，
而错一位要到收银台才会发现。

```
Chrome / Android   用浏览器自带的 BarcodeDetector，不下载任何东西
iOS Safari         没有这个 API -> 按需加载内置的 zxing 解码器（1 MB wasm）
相机用不了         还能「从相册选一张」，读文件不需要相机权限
```

**构造原生检测器之前必须先问它支持哪些格式**（`getSupportedFormats()`）——
传进去一个平台不支持的格式，构造器直接抛 `NotSupportedError`。
各家实现差别不小，PDF417 尤其参差。

判断标准是「必需的格式在不在」而不是「全都在」：macOS 上的 Chrome 就不认 `upc_a`，
但那是美国零售用的、这边基本见不到 —— 为它让每个人多下 1 MB 不值当。
缺了 PDF417 这种真会用到的才退到 wasm。

那 1 MB 的 wasm **不进预缓存**（`globPatterns` 里没有 `.wasm`）——
只有没原生支持的浏览器才用得上，让所有人装应用时都下载它太亏。
但扫过一次之后会被 CacheFirst 缓存住，超市里信号差也照样能扫。

> zxing-wasm 默认去 jsDelivr 拿 wasm。这是个自建应用，不该偷偷依赖外部 CDN，
> 而且那样一来离线就废了。所以用 vite 的 `?url` 把 wasm 打进产物，
> 再用 `setZXingModuleOverrides({ locateFile })` 指到自己服务器上。

相机需要**安全上下文**（https 或 localhost）。从局域网 IP 用明文 http 打开时
`getUserMedia` 根本不存在 —— 这时候会直接说清楚原因并让你走「从相册选一张」，
而不是让你点了按钮对着一片黑（同第 29 节的剪贴板、第 20 节的推送）。
...
装成 App 之后，**没网也能用**——但不同功能的离线程度不一样，这是刻意的。

```
读        全部可离线          菜单、菜谱、购物清单、记账、卡包
写-追加   离线攒着，联网补发   记一笔开销、勾购物清单
写-修改   要求联网            改菜谱、调菜单、改已记的账
```

**为什么修改不排队。** 追加是安全的：你在外面记一笔，家里人也记一笔，
两笔都成立，谁也不覆盖谁。修改不是 —— 你在地铁里改了菜谱，家人同时也改了，
等你联网补发就会无声地把对方的改动盖掉。这种冲突没有安全的自动解法
（真要解就得引入版本号和三方合并，对一个家庭应用来说代价远大于收益），
所以离线时直接告诉用户「改动需要联网」，而不是假装成功。

**幂等是补发的前提。** 手机没信号时请求可能已经到了、响应却丢了 ——
客户端不知道，只能重发。没有防重就会记成两笔：

```
记账     每条操作带一个客户端生成的 clientOpId，服务器认这个 id
         实测连发三次同一个 id -> 库里只有一条，后两次返回 duplicate: true

购物清单 改成「设成 true」而不是「翻转」
         翻转发三次 -> false, true, false   ← 老实现的 bug
         设值发三次 -> true, true, true
```

`shopping_list_items` 的勾选原来走 `/toggle`，补发时会翻回去 ——
「我明明勾上了，过一会儿自己没了」。现在用 `PATCH /api/shopping/item/:id { checked }`。

**卡包是本地优先的。** 站在收银台前不能等：打开就先用上次存的那份画出来，
同时后台刷新。NetworkFirst 断网要等 3 秒超时才回退到缓存，排队的人可等不了。
离线时会标一行「离线中，这是 x 分钟前同步的」——卡号不会变，照用无妨，但得让人知道。

**判断在不在线，不能只看 `navigator.onLine`。** 它说的是「有没有网卡连着」，
不是「够不着服务器」。实测断网冷启动时它报 `true` 而请求全部失败
（强制门户、弱信号、服务器挂了都是这样）。所以以**我们自己的请求**为准，
`navigator.onLine === false` 只当补充信号。

判断谁的「成功」算数，有个坑：断网时 Service Worker 会拿缓存顶上，
那也是 200。所以只认**从不进缓存的接口** —— `/api/auth/*` 明确不在缓存白名单里
（登录状态不该来自缓存），它答话了才说明真的够得着服务器。

> 试过让 SW 给缓存响应打个 `X-From-Cache` 头再由前端识别，
> 实测那个头到不了页面，就撤了。按路径判断更硬，也更好读。

**这个状态必须能自愈。** 早先的实现只在挂载时问一次 `/auth/me`，
失败就把离线标记置上，而且**只有那一次请求**能把它改回来 ——
iOS 上装到桌面后冷启动，网络栈往往还没就绪，第一次请求必然失败，
于是「离线中」就永远挂在那儿了，哪怕后面每个请求都是好的。

现在：**每个请求**都上报可达性（`lib/reachability.js`），谁成功都能拨回在线；
掉线期间还有心跳，3s → 6s → 12s → 20s 地探，
应用一直开着、网络自己恢复（地铁出站、Wi-Fi 重连）也能自己好。

**发不出去的操作不会被悄悄丢掉。** 服务器明确拒绝（4xx）或重试超过 5 次的，
会移出队列进「失败」列表并在顶栏红条里列出来 —— 关键是**必须移出队列**，
一条坏数据留在队首会把后面所有正常操作全堵死，用户永远同步不上还找不到原因。

补发的时机：联网事件、页面回到前台、应用启动。

> 一个前提：SW 里必须有 SPA 的导航路由（`NavigationRoute` + `createHandlerBoundToURL`）。
> 没有它的话，precache 只认 `/index.html`，直接打开 `/cards` 会走网络 ——
> 断网就是浏览器的小恐龙页面，整个离线能力等于零。这个坑本仓库踩过。
...
│   │   ├── cardBrands.js   常见商家预设（名字/品牌色/常见码制）
│   │   ├── expenseCategories.js 收/支两套分类 + 图标名
...
│   │   ├── expenseCalc.test.mjs  记账键盘的加减（不用 eval）
│   │   ├── syncQueue.test.mjs    离线队列的去重和失败分类
│   │   └── cardBrands.test.mjs   商家预设表的自检（颜色/码制/分组）
